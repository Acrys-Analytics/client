<script lang="ts">
  ...
  $: position =
    Position[
      summoner.clashPosition || summoner.mostPlayedPosition[0]?.position
    ] || Position.FILL;

  $: gamesPlayedWithPosition =
    summoner.mostPlayedPosition.length > 0
      ? summoner.mostPlayedPosition?.find(
          (value) => value.position === position
        ).count
      : 0;

  $: globalWinrate = (
    (summoner.globalStats?.wins / summoner.globalStats?.totalGames) *
    100
  ).toFixed(2);
</script>
...
<div class="min-w-[360px] flex flex-col">
  <div
    class="h-48 bg-img"
    style="background-image: url('https://ddragon.leagueoflegends.com/cdn/img/champion/splash/{mostPlayedChampion}_0.jpg');"
  />
  <div class="-mt-5 card flex flex-col items-center">
    ...
    <ChampionPool
      champions={summoner.championPool}
      summonerTotalGames={summoner.participants.length}
    />
